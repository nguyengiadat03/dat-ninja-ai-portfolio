...
import { MessageCircle, Send, X, Image, MoreHorizontal, Sparkles } from "lucide-react";
...
const ChatBot = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      content: "Xin chào! Tôi là ĐạtGPT 🤖 Tôi có thể giúp bạn tìm hiểu về Nguyễn Gia Đạt và chương trình Ninja AI. Bạn muốn biết điều gì?",
      isUser: false,
      timestamp: new Date(),
    },
  ]);
  ...
  const getAIResponse = (userMessage: string): string => {
    ...
    if (lowerMessage.includes("đạt") || lowerMessage.includes("nguyễn gia đạt")) {
    ...
    if (lowerMessage.includes("ninja ai") || lowerMessage.includes("chương trình")) {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <Card className="fixed bottom-6 right-6 w-80 h-96 z-50 flex flex-col bg-card/95 backdrop-blur-md border shadow-xl animate-scale-in">
          {/* Header */}
          <div className="flex items-center justify-between p-4 border-b border-border bg-gradient-primary text-white rounded-t-lg">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <h3 className="font-semibold">ĐạtGPT</h3>
              ...
              <div className="grid grid-cols-1 gap-1">
                {quickQuestions.map((question, index) => (
                  <Button
                    ...
                    className="text-xs h-auto p-2 justify-start text-left hover:bg-muted"
                  >
                    {question}
                  </Button>
                ))}
